...
g++ test-reader-pref.cpp rwlock-reader-pref.cpp -o rwlock-reader-pref.o -lpthread

echo "CASE1: Reader Preference with 5 reader and 1 writer"
total=$((total+1))
out=`./rwlock-reader-pref.o 5 1`
echo $out
if [ "$out" = "PASSED" ]; then
    correct=$((correct+1))
fi
...
g++ test-writer-pref.cpp rwlock-writer-pref.cpp -o rwlock-writer-pref.o -lpthread

echo "CASE1: Writer Preference with 5 reader and 1 writer"
total=$((total+1))
out=`./rwlock-writer-pref.o 5 1`
echo $out
if [ "$out" = "PASSED" ]; then
    correct=$((correct+1))
fi
