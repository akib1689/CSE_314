...
int main(int argc, char *argv[]) {
  ...
  long *max_reader_acquire_time_first_half =
      max_element(readerAcquireTime, readerAcquireTime + read_num_threads);
  long *min_reader_release_time_first_half =
      min_element(readerReleaseTime, readerReleaseTime + read_num_threads);
  long *max_reader_release_time_first_half =
      max_element(readerReleaseTime, readerReleaseTime + read_num_threads);
  long *min_reader_acquire_time_second_half =
      min_element(readerAcquireTime + read_num_threads,
                  readerAcquireTime + 2 * read_num_threads);
  long *max_reader_acquire_time_second_half =
      max_element(readerAcquireTime + read_num_threads,
                  readerAcquireTime + 2 * read_num_threads);
  long *min_reader_release_time_second_half =
      min_element(readerReleaseTime + read_num_threads,
                  readerReleaseTime + 2 * read_num_threads);
  long *min_writer_acquire_time =
      min_element(writerAcquireTime, writerAcquireTime + write_num_threads);
  long *max_writer_release_time =
      max_element(writerReleaseTime, writerReleaseTime + write_num_threads);

  // not necessary
  // // check if all readers get lock immediately in first half
  // if  ((read_num_threads > 0) && (*max_reader_acquire_time_first_half >
  // *min_reader_release_time_first_half)){ 	printf("Reader should not wait to
  // acquire lock in first half\n"); 	exit(0);
  // }

  // check if all readers get lock immediately in second half
  if ((read_num_threads > 0) && (*max_reader_acquire_time_second_half >
                                 *min_reader_release_time_second_half)) {
  ...
  // if  ((read_num_threads > 0) && (write_num_threads > 0) &&
  // (*max_reader_release_time_first_half > *min_writer_acquire_time)){
